...
description: 블로그 글을 인스타 카드뉴스로 자동 변환하기. AI 이미지 생성으로 글자를 넣으면 깨지는 문제를 HTML 템플릿+Chrome 헤드리스 캡처로 해결한 방법. (시리즈 2편)
---
...
블로그 글을 인스타그램 카드로 바꾸려면 카드에 **한글 텍스트**를 넣어야 한다. 처음엔 "어차피 이미지니까 AI로 카드를 통째로 생성하면 되겠지" 했는데 — **글자가 깨졌다.** 제목도 본문도, 한글이 알아볼 수 없는 형태로 뭉개졌다.

## 원인 — 왜 AI 생성 카드는 글자가 깨지나

이미지 생성 모델(FLUX 등)은 픽셀을 "그리는" 방식이라 **정확한 글자 형태를 만들지 못한다.** 영문도 자주 깨지고 **한글은 더 심하다.** 카드뉴스의 핵심은 "읽히는 텍스트"인데, 그게 깨지면 카드 자체가 무용지물이다.
...
- `--window-size=1080,1350` 로 정확한 카드 규격을 잡고
- `--virtual-time-budget` 으로 **웹폰트 로딩을 기다린다** (안 주면 폰트가 적용되기 전에 캡처돼서, HTML로 그렸는데도 글자가 기본 폰트로 나오거나 깨진다)
- Node 스크립트로 슬라이드 수만큼 반복 렌더 → 카드 6장을 한 번에 생성
...
- **HTML/CSS 템플릿** — 카드 레이아웃을 데이터 주입식으로
- **Pretendard** — 한글 웹폰트(일관·선명)
- **Node.js** — 슬라이드 JSON → 템플릿 → 반복 렌더 스크립트
- **로컬 FLUX** — 표지 배경 이미지(글자 없는 부분만)

## 정리

- 카드의 글자를 AI 이미지 생성으로 넣지 마라 — 깨진다.
- 텍스트는 HTML로 그리고 Chrome 헤드리스로 캡처 = 무료·선명·자동.
- 폰트 로딩 대기(`--virtual-time-budget`)만 빠뜨리지 말 것.
